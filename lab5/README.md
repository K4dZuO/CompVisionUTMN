...
- **Отслеживание объектов**: Детектирование и трекинг движущихся объектов (например, машин)
- **Интерактивный GUI**: Удобный интерфейс на PyQt5
- **Визуализация**: Различные режимы визуализации (HSV, стрелки, heat maps, bounding boxes)
...
3. **Выбор алгоритма**: Выберите "Хорн-Шанк" или "Лукас-Канаде" из выпадающего списка
4. **Настройка параметров**: Измените параметры алгоритмов с помощью слайдеров
5. **Включение отслеживания** (опционально): Поставьте галочку "Включить отслеживание" для детектирования и трекинга объектов
6. **Обработка**: Нажмите "Обработать кадр"
7. **Визуализация**: Выберите режим визуализации из выпадающего списка
8. **Экспорт**: Сохраните результаты анализа в виде отчёта

### Режимы работы

**Только оптический поток:**
- Визуализация движения в кадре
- Выбор между плотным (Хорн-Шанк) и разреженным (Лукас-Канаде) потоком

**Оптический поток + Отслеживание:**
- Включите чекбокс "Включить отслеживание"
- Система автоматически создаст маску движения на основе выбранного алгоритма потока
- Отобразятся bounding box'ы, ID и траектории движущихся объектов
- Настройте параметры трекинга (мин. площадь, макс. исчезновение, макс. дистанция)
...
**Метод:**
Комбинация вычитания фона (Background Subtraction) и центроидного отслеживания (Centroid Tracking).

1. **Детектирование:**
   - Используется `BackgroundSubtractorMOG2` для выделения движущихся объектов.
   - Применяются морфологические операции для удаления шума.
   - Выделяются контуры объектов, удовлетворяющие минимальной площади.

2. **Отслеживание:**
   - Для каждого кадра вычисляются центроиды обнаруженных объектов.
   - Центроиды сопоставляются с уже отслеживаемыми объектами на основе Евклидова расстояния.
   - Присваиваются уникальные ID, которые сохраняются между кадрами.

**Параметры:**
- `min_area`: Минимальная площадь объекта (в пикселях) для детектирования.
- `max_disappeared`: Количество кадров, которое объект может отсутствовать до потери трекинга.
- `max_distance`: Максимальное расстояние смещения центроида между кадрами.
