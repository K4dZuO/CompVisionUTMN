...
def logarithmic_transform(image: np.ndarray) -> np.ndarray:
    ...
    Коэффициент c подбирается так, чтобы исходный диапазон яркости был отображен в диапазон палитры.
    """
...
def power_transform(image: np.ndarray, gamma: float) -> np.ndarray:
    ...
    Коэффициент c подбирается автоматически.
    """
    image_float = image.astype(np.float64) / 255.0
    c = 1.0  # Для нормализованного изображения коэффициент обычно 1
...
def brightness_range_cutout(image: np.ndarray, min_val: int, max_val: int, 
                           constant_value: Optional[int] = None) -> np.ndarray:
    """
    Вырезание произвольного диапазона яркостей.
    Если constant_value указан, пиксели вне диапазона приводятся к этому значению.
    Иначе сохраняются в исходном виде.
    """
    ...
    if constant_value is not None:
        ...
        result = image.copy()
        result[~mask] = 0  # Можно изменить на другое значение при необходимости
...
def gaussian_filter(image: np.ndarray, sigma: float) -> np.ndarray:
    ...
    """
    # Определяем размер ядра по правилу 3σ
...
def unsharp_masking(image: np.ndarray, k: int, lambda_val: float) -> np.ndarray:
    ...
    # Вычисляем маску (разность между оригиналом и размытым)
    mask = image.astype(np.float64) - blurred.astype(np.float64)
    
    # Применяем маску с коэффициентом λ
...
def add_gaussian_noise(image: np.ndarray, mean: float = 0, sigma: float = 25) -> np.ndarray:
    """
    Добавление гауссова шума к изображению для тестирования фильтров.
    """
